				...
				_renderFile : function(file){
                    function getfilemd5(fileObj){
                        ...
                        fileReader.onload = function(e) {
                            ...
                            var _md5=spark.end();
                             $.ajax({
                             'url': '/api/check_file/md5',
                             'dataType': 'json',
                             'data': {md5: _md5,title:file.name,user_id:6},
                             'type': 'post',
                             'success': function (data) {
                             if (data.status == true){
                                 $("#upload_message").html("");
                                 $("#upload_message").html(file.name+"已经进行秒传");
                                 uploadManager._uploadedFile(file);
                                }
                             }
                             });
                        };
                        fileReader.readAsBinaryString(blobSlice.call(fileObj, 0, fileObj.size));
                    }
