...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
    function login($login, $password, $remember, $login_by_username, $login_by_email , $login_by_mobile ,$api = false)
    {
        if ((strlen($login) > 0) AND (strlen($password) > 0) ) {

            // Which function to use to login (based on config)
            if ($login_by_username AND $login_by_email AND $login_by_mobile) {
                $get_user_func = 'get_user_by_login';
            } else if ($login_by_username) {
                $get_user_func = 'get_user_by_username';
            } else if ($login_by_email){
    ...
    function logout()
    {
        ...
        $this->ci->session->set_userdata(array('login_user' => ''));
	...
	{
		$user_id = $this->ci->session->userdata('login_user')['user_id'];
 ...
 * return array
 */
    function set_activate_email_key($email){
        $data['email'] = $email;
        $data['email_activate_key'] = md5(rand().microtime());
        $flag = $this->ci->users->set_email_activate_key($email, $data['email_activate_key']);
        if(!$flag){
            return false;
        }else
            return $data;
    ...
    function activate_email_first($key){
        $user = $this->ci->users->activate_email_first($key,true,$this->ci->config->item('email_activation_expire', 'tank_auth'));
        //var_dump($user);
        if($user)
        {
            $this->ci->session->set_userdata('login_user',
    ...
    function send_verify_register($mobile)
    {
        if(!$this->ci->session->userdata($mobile)){
            $this->ci->session->set_userdata($mobile,time());
        }else{
            if(time()-$this->ci->session->userdata($mobile)<60){
                return '10404';
            }else{
                $this->ci->session->set_userdata($mobile,time());
            }
        ...
        if($flag){
            $this->ci->session->set_userdata('sms_verify_register',array('mobile'=>$mobile,'verify'=>md5($verify_code)));
            return '10405';
        }else{
            return '10406';
        }
    ...
    function get_session_register()
    {
        return $this->ci->session->userdata('sms_verify_register');
    }
    function get_session_resetpassword()
    {
        return $this->ci->session->userdata('sms_verify_resetpassword');
    }
}
