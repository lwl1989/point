...
//常量定义
//define('chr(0xFF).chr(0xFE)', chr(0xFF).chr(0xFE));
//define('UCS2', 'ucs-2be');
class Phpanalysis
{
    ...
    //private UCS2 = 'ucs-2be';
   // private  chr(0xFF).chr(0xFE) = chr(0xFF).chr(0xFE);
    //主词典 
    public $dicStr = '';
    ...
    {  //$source_charset='utf-8', $target_charset='utf-8', $load_all=true, $source=''
        //$CI =& get_instance();
        ...
        $this->SetSource( '', $config['source_charset'], $config['target_charset'] );
        //echo $source_charset;
        $this->isLoadAll = $config['load_all'];
        //if(self::$loadInit) 
	    $this->LoadDict();
        //echo '222';
    }
    ...
    public function SetSource( $source, $source_charset='utf-8', $target_charset='utf-8' )
    {
        ...
        if( $source != '' )
        {
            ...
            if( preg_match("/^utf/", $source_charset) ) {
                ...
                $this->sourceString = iconv('utf-8', 'ucs-2be', iconv('gb18030', 'utf-8', $source));
            }
            else if( preg_match("/^big/", $source_charset) ) {
                $this->sourceString = iconv('utf-8', 'ucs-2be', iconv('big5', 'utf-8', $source));
            }
            else {
                $rs = false;
            }
            ...
            if($cn < 0x80)
            {
                ...
                else
                {
                    if( $onstr != '' )
                    {
                        ...
                        if( $lastc==2 )
                        {
                            if( !preg_match('/'.$notNumberMatch.'/i', iconv('ucs-2be', 'utf-8', $onstr)) ) $lastc = 4;
                        }
                        $this->simpleResult[$s]['t'] = $lastc;
    ...
    */
     ...
     */
     private function _out_string_encoding( &$str )
     {
        ...
        if( $rsc==1 ) {
            $rsstr = iconv('ucs-2be', 'utf-8', $str);
        }
        else if( $rsc==2 ) {
            $rsstr = iconv('utf-8', 'gb18030', iconv('ucs-2be', 'utf-8', $str) );
        }
        else{
            $rsstr = iconv('utf-8', 'big5', iconv('ucs-2be', 'utf-8', $str) );
        }
        return $rsstr;
     }
