    ...
    <table class="table table-striped table-bordered">
        ...
        <?php if($pagination['list']): foreach($pagination['list'] as $vo):?>
            <tr id="data<?php echo $vo['id'];?>">
                ...
                <td id="close">
                    <?php if($vo['closed']){echo '封停至'.$vo['open_time'];}else{
                        echo '正常';
                    }?>
                </td>
                <td>
                ...
                <td>
                    <?php if($vo['closed']){?>
                        <a class="btn btn-info btn-mini"
                           ...
                           onclick="return reback(<?php echo $vo['id'];?>);">
                ...
                <?php }else{ ?>
                    <a class="btn btn-info btn-mini"
                       href="javascript:void(0)"
                        onclick="return stop(<?php echo $vo['id'];?>);">
...
<script  type="text/javascript">
    ...
    function del(id)
    {
        if(!confirm('确定要删除当前用户信息？'))
            return false;
        $.post(
             "<?php echo site_url('m-site/user/del')?>",
             { id: id, action: 'del' },
             function(response){
                 if(response.status) {
                     $('#data' + id).fadeOut();
    ...
    function stop(id){
        if(!confirm('确定要封停当前用户信息？'))
            return false;
        $.post(
             "<?php echo site_url('m-site/user/stop')?>",
             { id: id, action: 'update' },
             function(response){
                 if(response.status) {
                     //$('#data' + id).fadeOut();
                     $('#data'+id).find('#close').html('封停至'+response.msg);
                 }else if(!response.status){
                     alert(response.msg);
                 }else {
                     alert('操作失败，请重试或联系管理员！');
